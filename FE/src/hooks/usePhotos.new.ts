...
export const useSearchPhotos = (
  ...
  // Preload additional pages on initial load
  useEffect(() => {
    const currentPages = result.data?.pages.length || 0;
    const shouldPreload =
      currentPages > 0 &&
      currentPages < preloadPages &&
      result.hasNextPage &&
      !result.isFetchingNextPage;

    if (shouldPreload) {
      // Use setTimeout to avoid calling fetchNextPage during render
...
export const useInfinitePhotos = (
  ...
  if (searchQuery && typeof searchQuery === "string" && searchQuery.trim()) {
    return useSearchPhotos(searchQuery, perPage, preloadPages);
  }

  // If there's a topic, use topic photos
  if (topic && typeof topic === "string" && topic.trim()) {
    return usePhotosByTopic(topic, perPage, preloadPages);
  }
  ...
  // Preload additional pages on initial load for random photos
  useEffect(() => {
    const currentPages = result.data?.pages.length || 0;
    const shouldPreload =
      currentPages > 0 &&
